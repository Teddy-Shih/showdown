...
async function main() {
  ...
  console.log('TYPEAWAREBOT: TEAM COVERAGE & MATCHUP ANALYSIS');
  console.log('='.repeat(80));
  console.log();
  console.log('🆕 NEW FEATURES:');
  console.log('   - Team coverage analysis (resist/counter matrix) ✓');
  console.log('   - Best matchup finder (find optimal counter for each threat) ✓');
  console.log('   - Team health evaluation (preserve key Pokemon) ✓');
  console.log('   - Speed control & revenge killing maintained ✓');
  console.log('   - Boost propagation maintained (accurate to gameplay) ✓');
  console.log();
  console.log('TEAM COVERAGE:');
  console.log('   - Analyzes which Pokemon resist which types');
  console.log('   - Tracks offensive coverage across team');
  console.log('   - Identifies checks for opponent Pokemon');
  console.log();
  console.log('MATCHUP ANALYSIS:');
  console.log('   - +50 bonus for switching to best counter');
  console.log('   - +30 extra if strong type advantage (>2)');
  console.log('   - +20 extra if significant damage advantage (>30)');
  console.log('   - +15 if low on healthy Pokemon (≤3), preserve damaged ones');
  console.log();
  console.log('SPEED & REVENGE KILLING:');
  console.log('   - +150 if switch-in outspeeds and guarantees KO');
  console.log('   - Speed-aware move ordering (faster KO = 200k priority)');
  console.log('   - +0.2 per speed point, +15 if faster + super effective');
  console.log();
  console.log('WHY NO HEURISTICS:');
  console.log('   - ANY heuristic (+150, +20, +5) made performance worse');
  console.log('   - Heuristic at root conflicts with minimax evaluation');
  console.log('   - Pure search performs best: 73.3% vs 30% with +20 bonus');
  console.log();
  console.log('TESTING HISTORY:');
  console.log('   - Baseline (no setup): 86.7% (26-4)');
  console.log('   - Flat +150 bonus: 10% (catastrophic)');
  console.log('   - Propagation only: 73.3% (22-8) ← previous test');
  console.log('   - +20 heuristic: 30% (conflicts with search)');
  console.log('   - +5 heuristic: 30% (still conflicts)');
  console.log();
  console.log('EXPECTED: 73% win rate (confirming previous result)');
  console.log();
  console.log('='.repeat(80));
  ...
  console.log('='.repeat(80));
  console.log(`Baseline (no setup):                      86.7% (26-4)`);
  console.log(`Propagation only:                         73.3% (22-8)`);
  console.log(`Propagation + Heuristic (+20) (${numBattles}g):      ${currentOverall.toFixed(1)}% (${typeAwareWins}-${treeSearchWins})`);
  console.log(`Change from baseline:                     ${improvement >= 0 ? '+' : ''}${improvement.toFixed(1)} percentage points`);
  console.log(`Change from propagation-only:             ${(currentOverall - 73.3) >= 0 ? '+' : ''}${(currentOverall - 73.3).toFixed(1)} percentage points`);
  console.log();
