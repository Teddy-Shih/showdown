...
/**
 * ImprovedTypeAwareBot - TypeAwareBot with fixes from loss analysis
 *
 * Fixes Applied:
 * 1. More aggressive switch threshold (-3 → -2)
 * 2. Earlier HP-based switching (30 HP → 50 HP)
 * 3. Increased HP preservation bonus (80 → 120)
 * 4. Move diversity penalty for repetition
 * 5. Team HP total consideration
 * 6. "Being walled" detection
 */
class ImprovedTypeAwareBot {
  constructor(playerName) {
    ...
    // NEW: Track recent damage for wall detection
    this.recentDamageDealt = [];
    this.maxDamageHistory = 3;
  }
  ...
  processSwitch(line) {
    ...
    // Reset damage tracking on opponent switch
    this.recentDamageDealt = [];
  }
  ...
  processDamage(line) {
    ...
    if (this.isOpponent(target) && this.opponentActive) {
      const oldHP = this.opponentActive.hp;
      ...
      const damageDealt = oldHP - hp.current;
      if (damageDealt > 0) {
        this.recentDamageDealt.push(damageDealt);
        if (this.recentDamageDealt.length > this.maxDamageHistory) {
          this.recentDamageDealt.shift();
        }
      }
    }
  }
  ...
  chooseMove(request) {
    ...
    // IMPROVED: Check if we should switch with better logic
  ...
  /**
   * IMPROVED: More aggressive switch threshold
   */
  shouldProactivelySwitch(request, ourPokemon, ourHP) {
    ...
    // FIXED: More aggressive threshold (-3 → -2)
    if (typeMatchup < -2) {
      return true;
    }

    // FIXED: Earlier HP-based switching (30 → 50)
    if (ourHP < 50 && typeMatchup < 0) {
      return true;
    }

    // NEW: "Being walled" detection - minimal damage for recent turns
    if (this.recentDamageDealt.length >= 2) {
      const avgRecentDamage = this.recentDamageDealt.reduce((a, b) => a + b, 0) / this.recentDamageDealt.length;
      if (avgRecentDamage < 15 && typeMatchup <= 0) {
        return true; // We're doing minimal damage, should switch
      }
    }
  ...
  searchBestMove(request, availableMoves, ourPokemon, ourHP, oppHP) {
    ...
    for (let i = 0; i < availableMoves.length; i++) {
      ...
      // NEW: Apply move diversity penalty
      const diversityPenalty = this.getMoveDiversityPenalty(move.id);
      ...
      const score = this.minimaxDepth(
        ...
        moveData, null,
        1, alpha, beta, false,
        request.side.pokemon // Pass team for evaluation
      ) - diversityPenalty;
  ...
  /**
   * NEW: Calculate move diversity penalty
   */
  getMoveDiversityPenalty(moveId) {
    // Count how many times this move appears in recent history
    const recentMoveUses = this.moveHistory.filter(h => {
      const match = h.match(/move (\d+)/);
      if (!match) return false;
      // This is a simplified check - in practice would need to map slot to move ID
      return h.includes('move');
    }).length;

    // If we've used moves repeatedly (not switches), apply penalty
    if (recentMoveUses >= 3) {
      return 20; // Penalty for lack of diversity
    }
  ...
  /**
   * Calculate type matchup score (offensive and defensive)
   */
  ...
  /**
   * IMPROVED: Enhanced evaluation with team HP consideration
   */
  evaluatePosition(ourPokemon, ourHP, oppPokemon, oppHP, team = null) {
    ...
    // FIXED: Increased HP preservation bonus (80 → 120)
    const ourHPRatio = Math.max(0, ourHP) / 100;
    const oppHPRatio = Math.max(0, oppHP) / 100;

    score += (ourHPRatio ** 2) * 120;  // Increased from 80
    score -= (oppHPRatio ** 2) * 120;  // Increased from 80

    // Progress bonus (reward reducing opponent HP)
    score += (1 - oppHPRatio) * 50;

    // NEW: Team HP consideration
    if (team) {
      const teamHPTotal = team.reduce((sum, p) => {
        if (p.condition === '0 fnt') return sum;
        const hp = this.parseHP(p.condition).current;
        return sum + hp;
      }, 0);

      // Bonus for preserving team health
      score += teamHPTotal * 0.2;
    }
...
module.exports = ImprovedTypeAwareBot;
