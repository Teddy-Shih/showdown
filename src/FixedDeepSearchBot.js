...
/**
 * FixedDeepSearchBot - ImprovedDeepSearchBot with loop detection and search fixes
 * Fixes:
 * 1. CORRECTED loop detection logic (check BEFORE adding to history)
 * 2. Force minimum node evaluation to ensure search runs
 * 3. Debug logging for low node counts
 * 4. Better move diversity
 */
class FixedDeepSearchBot {
  ...
  chooseMove(request) {
    ...
    // FIX: Ensure opponent has moves before searching
    if (this.opponentActive.moves.size === 0) {
      // Re-estimate if somehow lost
    ...
    // FIXED LOOP DETECTION: Check BEFORE adding to history
    const proposedChoice = `move ${bestMove.slot}`;

    // Check if we're about to repeat
  ...
  /**
   * FIXED: Check if we're ABOUT TO loop (before adding to history)
   */
  isAboutToLoop(proposedMove) {
    if (this.moveHistory.length < 2) return false;

    // Check if last 2 moves were the same as what we're about to do
    const last2 = this.moveHistory.slice(-2);
    return last2.every(m => m === proposedMove);
  }

  /**
   * Find an alternative move, preferring different ones
   */
  findAlternativeMove(availableMoves, currentSlot) {
    // Build list of moves we haven't used recently
    ...
    for (let i = 0; i < availableMoves.length; i++) {
      ...
      if (slot !== currentSlot && !move.disabled && move.pp > 0) {
        // Prefer moves we haven't used recently
    ...
    // If all moves were used recently, just pick any different one
    for (let i = 0; i < availableMoves.length; i++) {
      const slot = i + 1;
      const move = availableMoves[i];
  ...
  searchBestMove(request, availableMoves, ourPokemon, ourHP, oppHP) {
    ...
    // FIX: Ensure opponent has moves
    if (oppMoveList.length === 0) {
      return { slot: 1, move: ourMoveList[0].move, score: 0 };
    }
  ...
  evaluatePosition(ourHP, oppHP) {
    ...
    const oppHPRatio = oppHP / 100;
    score += (1 - oppHPRatio) * 100;
...
module.exports = FixedDeepSearchBot;
