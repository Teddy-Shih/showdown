...
/**
 * OrderedDeepSearchBot - DeepSearchBot + move ordering only
 * Only change: Order moves before searching for better alpha-beta pruning
 */
class OrderedDeepSearchBot {
  ...
  /**
   * ORDER MOVES - NEW: Sort moves by estimated value for better pruning
   */
  orderMoves(moves, ourPokemon, oppPokemon, oppHP) {
    ...
    for (const move of moves) {
      ...
      // Prioritize KO moves highest
      if (damage >= oppHP) {
        orderScore = 100000 + damage;
      }
      // Then high damage moves
      else if (damage > 0) {
  ...
  searchBestMove(request, availableMoves, ourPokemon, ourHP, oppHP) {
    ...
    // ORDER moves before searching (NEW)
  ...
  // Keep original simple evaluation
...
module.exports = OrderedDeepSearchBot;
